...
namespace Paral
{
    public static class FormatHelper
    {
        ...
        public static string ExpectedToken(Type expected, Token actual) => string.Format(_LEXER_ERROR_TEMPLATE, actual.Location.X, actual.Location.Y,
            string.Format(_EXPECTED_TOKEN_FORMAT, expected.Name, actual.GetType().Name));

        public static string UnexpectedToken(Token token) => string.Format(_LEXER_ERROR_TEMPLATE, token.Location.X, token.Location.Y, "Unexpected token.");
